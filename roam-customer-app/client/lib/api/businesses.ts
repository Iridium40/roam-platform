...
class BusinessesAPI extends BaseAPI {
  ...
  async getFeaturedBusinesses(accessToken?: string): Promise<BusinessRecord[]> {
    const response = await fetch(
      `${this.baseURL}/rest/v1/business_profiles?is_featured=eq.true&is_active=eq.true&verification_status=eq.approved&bank_connected=eq.true&stripe_account_id=is.not.null&providers.is_active=eq.true&providers.provider_services.is_active=eq.true&providers.provider_role=in.(owner,provider)&select=*,business_locations(location_name,city,state),providers!inner(id,provider_role,is_active,provider_services!inner(id,is_active))&limit=12`,
      {
        method: "GET",
  ...
  async getBusinessById(businessId: string, accessToken?: string): Promise<BusinessRecord> {
    const response = await fetch(
      `${this.baseURL}/rest/v1/business_profiles?id=eq.${businessId}&is_active=eq.true&verification_status=eq.approved&bank_connected=eq.true&stripe_account_id=is.not.null&providers.is_active=eq.true&providers.provider_services.is_active=eq.true&providers.provider_role=in.(owner,provider)&select=*,business_locations(*),services(*),providers!inner(*,provider_services!inner(id,is_active))`,
      {
        method: "GET",
        headers: this.getHeaders(true, accessToken),
      },
  ...
  async searchBusinesses(query: string, accessToken?: string): Promise<BusinessRecord[]> {
    const response = await fetch(
      `${this.baseURL}/rest/v1/business_profiles?is_active=eq.true&verification_status=eq.approved&bank_connected=eq.true&stripe_account_id=is.not.null&providers.is_active=eq.true&providers.provider_services.is_active=eq.true&providers.provider_role=in.(owner,provider)&or=(business_name.ilike.%${query}%,business_description.ilike.%${query}%,business_type.ilike.%${query}%)&select=*,business_locations(location_name,city,state),providers!inner(id,provider_role,is_active,provider_services!inner(id,is_active))`,
      {
        method: "GET",
        headers: this.getHeaders(true, accessToken),
      },
