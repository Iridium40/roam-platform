...
export const updateCustomerProfileViaEdgeFunction = async (
  ...
  logger.debug("Attempting customer profile update", {
    customerId,
    updateData,
  ...
  try {
    logger.debug("Trying Edge Function approach...");
    ...
    logger.debug("Successfully updated customer profile via Edge Function");
    ...
    if (
      ...
      logger.debug("Using REST API fallback approach...");
      ...
      try {
        ...
        if (checkResponse.ok) {
          try {
            ...
            logger.warn("Error parsing check response:", parseError);
          }
        }
        ...
        logger.error("REST API fallback also failed:", restError);
        throw new Error(
