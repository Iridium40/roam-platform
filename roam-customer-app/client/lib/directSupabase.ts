...
class DirectSupabaseAPI {
  ...
  async getSession(): Promise<{ user: { id: string; email: string } } | null> {
    try {
      ...
      if (!this.accessToken) {
        logger.debug("No access token available for session check");
        return null;
      }
  ...
  async testBucketAccess(bucket: string): Promise<boolean> {
    try {
      ...
      logger.debug("Bucket test response:", {
      ...
      logger.error("Bucket test error:", error);
      return false;
    }
  }
  ...
  async uploadFile(
    ...
    logger.debug("Upload request debug:", {
      bucket,
      path,
      fileName: file.name,
      fileSize: file.size,
    ...
    logger.debug("Upload response debug:", {
      status: response.status,
      statusText: response.statusText,
    ...
    if (!response.ok) {
      ...
      try {
        ...
        logger.warn("Could not read error response body:", readError);
      }
    ...
    try {
      ...
      logger.warn("Could not parse upload response:", parseError);
  ...
  async updateCustomerProfile(
    ...
    logger.debug("DirectSupabase updateCustomerProfile: Starting update", {
      customerId,
      updateData,
      baseURL: this.baseURL,
      hasApiKey: !!this.apiKey,
    ...
    logger.debug("DirectSupabase updateCustomerProfile: Check response", {
    ...
    logger.debug("DirectSupabase updateCustomerProfile: Response", {
      status: response.status,
      statusText: response.statusText,
      responseText,
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        ...
        try {
          ...
          logger.warn("Error processing response text:", stringifyError);
          errorMessage = `HTTP ${response.status} - ${response.statusText}`;
        }
  ...
  async createCustomerProfileRecord(
    ...
    logger.debug("DirectSupabase createCustomerProfileRecord: Response", {
      status: response.status,
      statusText: response.statusText,
      responseText,
      ok: response.ok,
