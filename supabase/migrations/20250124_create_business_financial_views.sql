-- Create views for business financial summaries
-- These views aggregate data from business_payment_transactions for easier querying
...
COMMENT ON VIEW business_earnings_summary IS 'Aggregated earnings summary per business per tax year';
...
COMMENT ON VIEW business_earnings_by_period IS 'Earnings aggregated by day, month, and year for trend analysis';

-- View 3: Business Earnings with Booking Details (for detailed reporting)
CREATE OR REPLACE VIEW business_earnings_detailed AS
SELECT 
    bpt.id,
    bpt.business_id,
    bpt.booking_id,
...
COMMENT ON VIEW business_earnings_detailed IS 'Detailed earnings view with booking, service, and customer information';
...
COMMENT ON VIEW business_earnings_by_service IS 'Earnings aggregated by service for performance analysis';
...
COMMENT ON VIEW business_monthly_earnings IS 'Monthly earnings summary for trend analysis and YTD calculations';

-- Create indexes to support these views (if not already exist)
CREATE INDEX IF NOT EXISTS idx_business_payment_transactions_business_payment_date 
ON business_payment_transactions(business_id, payment_date DESC);

CREATE INDEX IF NOT EXISTS idx_business_payment_transactions_business_tax_year 
ON business_payment_transactions(business_id, tax_year);

CREATE INDEX IF NOT EXISTS idx_business_payment_transactions_transaction_type 
ON business_payment_transactions(transaction_type);
