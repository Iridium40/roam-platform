-- Fix approve_and_activate_business RPC: avoid MIN(uuid) (not supported in some environments)
-- and still enforce the invariant that exactly 1 owner provider exists.
...
BEGIN
  ...
  -- Fetch the (single) owner provider id without relying on MIN(uuid)
  SELECT p.id
  INTO v_owner_provider_id
  FROM providers p
  WHERE p.business_id = p_business_id
    AND p.provider_role = 'owner'
  ...
  IF v_owner_provider_id IS NULL THEN
    RAISE EXCEPTION
      USING
        MESSAGE = 'Missing owner provider',
        DETAIL = 'Owner provider row exists but could not be selected.',
        ERRCODE = 'P0001';
  END IF;
...
-- Keep execute permissions consistent
GRANT EXECUTE ON FUNCTION public.approve_and_activate_business(uuid, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.approve_and_activate_business(uuid, uuid, text) TO service_role;
